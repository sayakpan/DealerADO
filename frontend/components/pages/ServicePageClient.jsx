...
import { generatePdf, submitServiceData } from "@/services/services"
import { validators } from "@/utils/validations"
import RenderedLogClient from "@/components/ui/RenderedLogClient"
import { toast } from "@/plugin/toast"
import Popover from "@/components/ui/Popover"
import SampleResponseDialog from "@/components/ui/SampleResponseDialog" // Import custom dialog instead of shadcn dialog
import { Info } from "lucide-react"

const ServicePageClient = ({ service, slug }) => {
    const [formData, setFormData] = useState({})
    const [showSampleResponseModal, setShowSampleResponseModal] = useState(false)
    const [sampleResponseData, setSampleResponseData] = useState(null)
    ...
    const handleInputChange = (fieldKey, value) => {
        ...
        // If auto_capitalize is true, force uppercase
        if (field?.auto_capitalize) {
    ...
    const validateForm = () => {
        ...
        if (service?.or_groups) {
            service.or_groups.forEach((group) => {
                const hasAtLeastOneField = group.fields.some(
                    (fieldKey) => formData[fieldKey] && formData[fieldKey].toString().trim() !== "",
                )
    ...
    const handleShowSampleResponse = () => {
        setSampleResponseData({ rendered: service.sample_response })
        setShowSampleResponseModal(true)
    }
    ...
    return (
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="flex flex-col lg:flex-row justify-center items-center lg:items-start gap-8">
                    ...
                    {service?.form_fields && service.form_fields.length > 0 && (
                        <form
                            ...
                            className="w-full md:max-w-[80%] lg:max-w-[49%] mx-auto lg:mx-0 px-5 py-7 bg-white rounded-[20px] shadow-[0px_4px_40px_5px_rgba(0,0,0,0.08)] flex flex-col justify-start items-start gap-4"
                        >
                            <div className="w-full flex flex-col gap-4">
                                        ...
                                        <div className="w-full p-3 border-b border-stone-300 flex flex-col justify-start items-start gap-2.5">
                                            <div className="text-zinc-500 text-sm md:text-sm font-normal flex items-center gap-1">
                                                ...
                                                {field.help_text && (
                                                    <Popover
                                                        trigger={
                                                            <span className="ml-1 mt-1 flex items-center cursor-pointer text-blue-500 text-xs">
                                            ...
                                            {formErrors[field.key] && (
                                                <div className="text-[#B52628] text-[10px] md:text-sm font-normal">{formErrors[field.key]}</div>
                                            )}
                                        </div>
                                ...
                                {/* Submit Button */}
                                <div
                                    className={`w-full h-12 p-2.5 rounded-2xl shadow-[0px_4px_16px_0px_rgba(0,0,0,0.20)] flex justify-center items-center gap-2.5 transition-colors duration-200 ${submitting ? "bg-gray-400 cursor-not-allowed" : "bg-[#B52628] hover:bg-[#9e1f21] cursor-pointer"
                                        }`}
                                >
                                    <button
                                        type="submit"
                    ...
                    <div className="w-full md:max-w-[80%] lg:max-w-[49%] mx-auto lg:mx-0 flex flex-col gap-6">
                        <div className="flex justify-between items-center w-full">
                            ...
                            <button
                                onClick={handleShowSampleResponse}
                                className="flex items-center gap-1 px-3 py-2 rounded-lg bg-[#B52628] hover:bg-[#9e1f21] text-white text-sm"
                            >
            ...
            <SampleResponseDialog
                isOpen={showSampleResponseModal}
                onClose={() => setShowSampleResponseModal(false)}
                sampleResponseData={sampleResponseData}
